...
jest.mock('../../api/hotel', () => ({ getHotelList: jest.fn(() => mockedResponse) }))

describe('Testing of the Hotels Page', () => {
	let context;
	beforeEach(() => {
		context = shallow(<Results />);
	});

	test('If all 3 filters are rendered', () => {
		expect(context.contains('Hotel Facilities')).toEqual(true);
		expect(context.contains('Rating')).toEqual(true);
		expect(context.contains('Price')).toEqual(true);
	})

	test('That no Hotel Facilities filters are rendered when hotel facilities section is collapsed', () => {
		const hotelFacilitiesItem = context.find('#hotelFacilitiesItem')

		expect(hotelFacilitiesItem.children()).toHaveLength(0);
	})

	test('If Hotel Facilities filters are rendered when hotel facilities are expanded', async () => {
	...
	test('Total number of hotels found title', () => {
		const hotelListTitle = context.find('.totalResultsTitle')

		expect(hotelListTitle.text()).toEqual(`${mockedResponse.holidays.length} holiday found`);
	})

	test('Total number of <Hotel> components rendered', () => {
		const listOfHotels = context.find('#listOfHotels')

		expect(listOfHotels.children()).toHaveLength(mockedResponse.holidays.length);
	})
});
